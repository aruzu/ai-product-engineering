...
def process_reviews(reviews_path: str, output_path: str, max_tokens: int = 4000):
    ...
    print(f"Saved {len(all_raw_feature_requests)} raw feature requests to {tmp_features_path}")
    
    # Group and refine feature requests
    print("Grouping and refining feature requests...")
    refined_features = group_and_refine_features(all_raw_feature_requests, client=openai_client)
...
def main():
    ...
    parser.add_argument("--reviews-path", type=str, required=True,
                        help="Path to the JSON file containing reviews")
    parser.add_argument("--output-path", type=str, default="feature_requests_interview_plan.json",
