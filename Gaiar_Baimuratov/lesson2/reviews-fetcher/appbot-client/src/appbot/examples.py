...
def get_app_reviews(app_id=None):
    ...
    if not app_id:
        print(
            "No app ID provided. Please set APPBOT_APP_ID in .env or provide as argument."
        )
        return
...
def get_sentiment_analysis(app_id=None):
    ...
    client = AppBotClient()
    breakdown = client.get_sentiment_breakdown(
    ...
    if total > 0:
        print(
            f"  Positive: {results.get('positive', 0)} ({results.get('positive', 0)/total*100:.1f}%)"
        )
        print(
            f"  Negative: {results.get('negative', 0)} ({results.get('negative', 0)/total*100:.1f}%)"
        )
        print(
            f"  Neutral: {results.get('neutral', 0)} ({results.get('neutral', 0)/total*100:.1f}%)"
        )
        print(
            f"  Mixed: {results.get('mixed', 0)} ({results.get('mixed', 0)/total*100:.1f}%)"
        )
    else:
...
def get_popular_topics(app_id=None):
    ...
    for word in words.get("results", [])[:10]:  # Show top 10 words
        ...
        print(
            f"    (positive: {word.get('positive')}, negative: {word.get('negative')})"
        )
