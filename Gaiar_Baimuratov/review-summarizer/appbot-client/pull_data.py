...
def pull_reviews(app_id, start_date, end_date, output_format='json'):
    ...
    while page <= total_pages:
        ...
        print(f"Fetched page {page} of {total_pages} ({len(reviews.get('results', []))} reviews)")
...
def pull_sentiment(app_id, start_date, end_date):
    ...
    save_to_json(
        sentiment, 
        f"results/sentiment_breakdown_{app_id}_{timestamp}.json"
    )
    
    # Save timeline
    save_to_json(
        timeline, 
        f"results/sentiment_timeline_{app_id}_{timestamp}.json"
    )
...
def pull_popular_words(app_id, start_date, end_date):
    ...
    save_to_json(
        popular_words, 
        f"results/popular_words_{app_id}_{timestamp}.json"
    )
    
    # Save critical words
    save_to_json(
        critical_words, 
        f"results/critical_words_{app_id}_{timestamp}.json"
    )
...
def main():
    ...
    parser.add_argument('--days', type=int, default=30, help='Number of days to pull data for (default: 30)')
    ...
    if not args.app_id:
        ...
        if not app_id:
            print("Error: No app ID provided. Please specify --app-id or set APPBOT_APP_ID in .env")
            return
    else:
        app_id = args.app_id
